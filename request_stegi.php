...
function getUrlslug($str, $options = array()) {
		...
		$startDate = @$_REQUEST['date_from'];
		//$action_date_start = new DateTime($startDate);
		//$action_date_start_new=$action_date_start->format('d/m/Y H:i');
		$endDate = @$_REQUEST['date_to'];
		//$action_date_end = new DateTime($endDate);
		$action_date_start_comp=DateTime::createFromFormat('d/m/Y H:i', $startDate);
		$action_date_end_comp=DateTime::createFromFormat('d/m/Y H:i', $endDate);
		if($action_date_start_comp>=$action_date_end_comp){
			echo 3;
			die;
		}
	...
	if(@$_REQUEST['activity_description']!='' && @$_REQUEST['activity_title']!='' && DateTime::createFromFormat('d/m/Y H:i', $startDate) && DateTime::createFromFormat('d/m/Y H:i', $endDate)){
		...
		$end_array=explode(' ',@$_REQUEST['date_to']);
		$end_array1=explode('/',$end_array[0]);
		$action_date_end=$end_array1[2].'-'.$end_array1[1].'-'.$end_array1[0].' '.$end_array[1].':00';

		$query_stegi="INSERT INTO #__stegihours VALUES ('','',0,'".$team_id."','".addslashes(@$_REQUEST['activity_title'])."','".getUrlslug(@$_REQUEST['activity_title'])."','','".addslashes(@$_REQUEST['activity_description'])."','".$action_date_start."','".$action_date_end."','0',1,1,1,'*','".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."','".$user->id."','".$user->id."','','','') ";
		...
		synathina_email('stegi_created_admin',$s_array,$emails,$att);
		//email to user
    $user = JFactory::getUser();
    $user_email = [$user->email];
		...
		synathina_email('stegi_created_user',$s_array,$user_email,$att);
		echo 1;
	}else{echo 2;}
}else{
