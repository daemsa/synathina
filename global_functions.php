<?php
function synathina_email($type,$parameters,$emails,$title,$attachment){
	$db = JFactory::getDbo();
	...
	foreach($custom_emails as $custom_email){	
		...
		$body = '<body style="margin:0px auto; padding:0px; background-color:#FFFFFF; color:#5d5d5d; font-family:Arial; outline:none; font-size:12px;" bgcolor="#FFFFFF">
							<div style="background-color:#FFFFFF;margin:0px auto; font-family:Arial;color:#5d5d5d;">
								...
								<div style="font-size: 18px;font-weight:bold; color:#05c0de;padding-bottom: 10px;">'.$title.'</div>';
		$s_array=array();
		////$s_array=array('%s1','%s2');
...
}function get_first_num_of_words($string, $num_of_words){	$string = preg_replace('/\s+/', ' ', trim($string));	$words = explode(" ", $string);	if ($num_of_words > count($words)) {		$num_of_words = count($words);	}	$new_string = "";	for ($i = 0; $i < $num_of_words; $i++) {		$new_string .= $words[$i] . " ";	}	return trim($new_string);}
?>