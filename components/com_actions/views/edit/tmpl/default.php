              ...
              foreach($messageDivArray as $parentKey => $subArray) {
                ...
                if(count($subArray['children'])){
                  foreach($subArray['children'] as $child) { ?>
                      <div class="form-group form--padded donation-message child-donation-message textarea-donation-<?php echo $parentKey."-".$child[1] ?>" <?php if(isset($messagesToSupporters[$child[1]]) && $messagesToSupporters[$child[1]] == '') { echo 'style="display:none"'; } ?>>
                          <label for="support_message" class="is-block">Μήνυμα προς <?php echo $child[0] ?>*:</label>
                          <textarea class="form-control max-600" id="support_message-<?php echo $parentKey."-".$child[1] ?>" rows="8" name="support_message-<?php echo $parentKey."-".$child[1] ?>" <?php if(isset($messagesToSupporters[$child[1]]) && $messagesToSupporters[$child[1]] != ''){ echo 'required="required"'; } ?>><?php echo (isset($messagesToSupporters[$child[1]]) ? $messagesToSupporters[$child[1]] : ''); ?></textarea>
                      </div>
                  <?php }
                } else { ?>
