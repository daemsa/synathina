...
.gm-style-mtc {
  display: none;
      ...
      #type-selector label {
		...
		<script type="text/javascript">	
			function updateParent() {					if(document.getElementById('pac-input').value==''){						alert('Παρακαλώ πληκτρολογήστε μία διεύθυνση');						return false;					}					if(document.getElementById('pac-input').value!='' && document.getElementById('lat').value=='37.980510' && document.getElementById('lng').value=='23.726836'){						alert('Παρακαλώ πληκτρολογήστε μία έγκυρη διεύθυνση');						return false;					}				
					opener.document.create_action.<?php echo @$_REQUEST['lng_name']; ?>.value = document.search.lng.value;
					//console.log(document.search.lng.value);
					opener.document.create_action.<?php echo @$_REQUEST['lat_name']; ?>.value = document.search.lat.value;
					opener.document.create_action.<?php echo @$_REQUEST['address_name']; ?>.value = document.getElementById('pac-input').value;										//opener.document.getElementById('show_frame').innerHTML = frame1;
					self.close();
					return false;
			}
		</script>	
  ...
  <body>
    <input id="pac-input" class="controls" type="text"  placeholder="Αναζητήστε τη διεύθυνση της δράσης σας" value="<?=(@$_REQUEST['address']!=''?@$_REQUEST['address']:'')?>" style="width:50%;" >
		...
		<form id="search" name="search" onsubmit="return updateParent();">
			...
			<input type="submit" name="submit" value="Αποθήκευση" style="margin-top:8px; padding:8px; cursor:pointer;" />
		</form>
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAYNUUjQIPltXiT_wygrj3gk9DCGV26zxg&libraries=places&callback=initMap"
        async defer></script>
  </body>
</html>
