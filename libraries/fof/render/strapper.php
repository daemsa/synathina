...
class FOFRenderStrapper extends FOFRenderAbstract
{
	...
	public function preRender($view, $task, $input, $config = array())
	{
		...
		{
			// @TODO: Remove the frontend Core version classes in FOF 3
			$classes = array(
				'joomla-version-' . $majorVersion,
				'joomla-version-' . $minorVersion,
			);
...
JS;
	...
	/**
	 * Renders the submenu (link bar) using Core's style. On Core 2.5 this
	 * is a list of bar separated links, on Core 3 it's a sidebar at the
...
HTML;
		...
		if ($show_header || $show_filters)
		{
			foreach ($headerFields as $headerField)
			{
				...
				// Under Core < 3.0 we can't have filter-only fields
		...
		if ($count = count($items))
		{
			...
			foreach ($items as $i => $item)
			{
				...
				foreach ($fields as $field)
				{
					...
					$labelClass = $field->labelClass ? $field->labelClass : $field->labelclass; // Core 2.5/3.x use different case for the same name
		...
		// Close the wrapper element div on Core 3.0+
		...
		// The id field is required in Core 3 front-end to prevent the pagination limit box from screwing it up. Huh!!
...
JS;
			...
			{
				// Core 2.5 has a conflict with the jQueryUI tooltip, therefore we
				// have to use native Core 2.5 tooltips
				JHtml::_('behavior.tooltip');
