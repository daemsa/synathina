...
abstract class FOFUtilsInstallscript
{
	...
	/**
	 * The minimum Core version required to install this extension
	 *
	 * @var   string
	 */
	...
	/**
	 * The maximum Core version this extension can be installed on
	 *
	 * @var   string
	 */
	...
	/**
	 * Post-installation message definitions for Core 3.2 or later.
	 *
	 * This array contains the message definitions for the Post-installation Messages component added in Core 3.2 and
	...
	/**
	 * Core pre-flight event. This runs before Core installs or updates the component. This is our last chance to
	 * tell Core if it should abort the installation.
	 *
	...
	public function preflight($type, $parent)
	{
		...
		// Check the minimum Core version
		if (!empty($this->minimumJoomlaVersion) && !version_compare(JVERSION, $this->minimumJoomlaVersion, 'ge'))
		{
			$msg = "<p>You need Core $this->minimumJoomlaVersion or later to install this component</p>";
		...
		// Check the maximum Core version
		if (!empty($this->maximumJoomlaVersion) && !version_compare(JVERSION, $this->maximumJoomlaVersion, 'le'))
		{
			$msg = "<p>You need Core $this->maximumJoomlaVersion or earlier to install this component</p>";
	...
	/**
	 * Runs after install, update or discover_update. In other words, it executes after Core has finished installing
	...
	public function postflight($type, $parent)
	{
		...
		// Make sure the Core menu structure is correct
		$this->_rebuildMenu();

		// Add post-installation messages on Core 3.2 and later
	...
	public function uninstall($parent)
	{
		...
		// Uninstall post-installation messages on Core 3.2 and later
	...
	/**
	 * Copies the CLI scripts into Core's cli directory
	 *
	...
	/**
	 * Core 1.6+ bugfix for "Can not build admin menus"
	 */
	 ...
	 * @return bool
	 *
	 * @throws Exception When the Core menu is FUBAR
	 */
	private function _createAdminMenus($parent)
	{
		...
		// We need to insert the menu item as the last child of Core's menu root node. By default this is the
	...
	/**
	 * Tells Core to rebuild its menu structure to make triple-sure that the Components menu items really do exist
	...
	/**
	 * Adds or updates a post-installation message (PIM) definition for Core 3.2 or later. You can use this in your
	 ...
	 *                        should be shown to the user. @see FOFTemplateUtils::parsePath() for RAD path format. Core
	 ...
	 *
	 * @see                   FOFTemplateUtils::parsePath() for RAD path format. Core will include this file
	...
	/**
	 * Applies the post-installation messages for Core 3.2 or later
	 *
	 * @return void
	 */
	protected function _applyPostInstallationMessages()
	{
		// Make sure it's Core 3.2.0 or later
