...
/**
 * A helper Model to interact with Core's extensions update feature
 */
class FOFUtilsUpdate extends FOFModel
{
	/** @var JUpdater The Core updater object */
	...
	/**
	 * Refreshes the Core update sites for this extension as needed
	 *
	 * @return  void
	 */
	public function refreshUpdateSite()
	{
		...
		else
		{
			...
			foreach ($updateSiteIds as $id)
			{
				...
				// Is it enabled (Core seriously sucks: IT DISABLES UPDATE SITES WITHOUT THE POSSIBILITY TO RE-ENABLE THEM!)
				if ($aSite->enabled)
				{
					...
					if (($aSite->name == $update_site['name']) && ($aSite->location == $update_site['location']))
					{
						...
						{
							// Core 3.1 or earlier. Updates may or may not work.
							continue;
						}
					}
				}
