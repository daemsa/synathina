...
/**
 * A helper class which provides update information for the Core CMS itself. This is slightly different than the
 * regular "extension" files as we need to know if a Core version is STS, LTS, testing, current and so on.
 */
class FOFUtilsUpdateJoomla extends FOFUtilsUpdateExtension
{
	 ...
	 * @param   string  $jVersion  Core version to fetch updates for, or null to use JVERSION
	 *
	 ...
	 * @param   string $currentVersion The current Core version number
	 *
	 ...
	 * @param   string  $jVersion  The current Core version number
	 *
	...
	/**
	 * Core has a lousy track record in naming its alpha, beta and release
	...
	/**
	 * Reloads the list of all updates available for the specified Core version
	 * from the network.
	 *
	 * @param    array   $sources   The enabled sources to look into
	 * @param    string  $jVersion  The Core version we are checking updates for
	 *
