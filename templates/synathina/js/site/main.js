      ...
      .each(function() {
          ...
          if (match.length == 3) {
             //this.id = match[1] + (cloneIndex)
					...
					var n = id.indexOf("activity");
					var l = id.indexOf("labelact");
					//alert(n)
					if(n==0){
						if(cloneIndex>1){
							this.id = id.substring(0,id.length-2)+ '_' + (cloneIndex)
						}else{
							this.id = id+ '_' + (cloneIndex)
						}
					}
					if(l==0){
						if(cloneIndex>1){
							//this.for = for.substring(0,for.length-2)+ '_' + (cloneIndex)
							this.setAttribute("for", 'activity'+id.substring(id.length-2)+ '_' + (cloneIndex));
						}else{
							//this.for = for+ '_' + (cloneIndex)
							this.setAttribute("for", 'activity'+id.substring(id.length-2)+ '_' + (cloneIndex));
						}
					}					
      });
