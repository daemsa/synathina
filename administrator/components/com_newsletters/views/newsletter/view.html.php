...
class NewslettersViewNewsletter extends JViewLegacy
{
	...
	protected function addToolbar()
	{
		...
		$canDo		= JHelperContent::getActions('com_newsletters', 'category', $this->item->id);

		JToolbarHelper::title('Newsletter');

		// Build the actions for new and existing records.
		if ($isNew)
		{
			// For new records, check the create permission.
			//if ($isNew && (count($user->getAuthorisedCategories('com_newsletters', 'core.create')) > 0))
			//{
		...
		else
		{
			...
			//{
			//	JToolbarHelper::versions('com_newsletters.newsletter', $this->item->id);
			//}
