...
class ActionsViewAction extends JViewLegacy
{
	...
	protected function addToolbar()
	{
		...
		$canDo		= JHelperContent::getActions('com_actions', 'category', $this->item->id);

		JToolbarHelper::title('Action');

		// Build the actions for new and existing records.
		if ($isNew)
		{
			// For new records, check the create permission.
			//if ($isNew && (count($user->getAuthorisedCategories('com_actions', 'core.create')) > 0))
			//{
		...
		else
		{
			...
			//{
			//	JToolbarHelper::versions('com_actions.action', $this->item->id);
			//}
