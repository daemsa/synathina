...
//JLoader::register('ContactHelper', JPATH_ADMINISTRATOR . '/components/com_actions/helpers/contact.php');
 ...
 * @since  1.6
 */
class ActionsModelAction extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_actions.action';
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_actions.action.' . (int) $record->id);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.edit.state', 'com_actions.action.' . (int) $record->id);
		}
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'Action', $prefix = 'ActionsTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_actions.action', 'action', array('control' => 'jform', 'load_data' => $loadData));
	...
	public function getItem($pk = null)
	{
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_actions', '#__actions', 'com_actions.action', $item->id, 'id', '', '');
		...
		if (!empty($item->id))
		{
			...
			$item->tags->getTagIds($item->id, 'com_actions.action');
		}
		$item->org_donation=ActionsModelAction::getOrgsdonation();
		$item->activities=ActionsModelAction::getActivities();
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_actions.edit.action.data', array());
		...
		$this->preprocessData('com_actions.action', $data);
	...
	{
		include('components/com_actions/Thumbnailer.premium.php');
		$thumbWidth=185;
		$thumbHeight=185;
		$realPath=str_replace("administrator", "", JPATH_BASE);
		$thumbPath=$realPath."images/action_logos/thumbs";
		...
		if (parent::save($data))
		{
			...
			if (!file_exists($config->get( 'abs_path' ).'/images/action_photos/'.$id)) {
				mkdir($config->get( 'abs_path' ).'/images/action_photos/'.$id, 0777);
			}
			if (!file_exists($config->get( 'abs_path' ).'/images/action_files/'.$id)) {
				mkdir($config->get( 'abs_path' ).'/images/action_files/'.$id, 0777);
			}			
			...
			if ($assoc)
			{
				...
				$query = $db->getQuery(true)
					...
					->where('context=' . $db->quote('com_actions.action'))
				...
				if (!$all_language && count($associations))
				{
					...
					{
						$query->values($id . ',' . $db->quote('com_actions.action') . ',' . $db->quote($key));
					}
		...
		if ($assoc)
		{
			...
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) || $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_action');
...
/*	public function __construct($db)
	...
	public function bind($array, $ignore = '')
	{
		...
		$k = $this->_tbl_key;
		return 'com_actions.action.'.(int) $this->$k;
        }
 
	protected function _getAssetParentId($table = null, $id = null)
	{
		$asset = JTable::getInstance('Asset');
		$asset->loadByName('com_actions');
		return $asset->id;
	}	*/
}
