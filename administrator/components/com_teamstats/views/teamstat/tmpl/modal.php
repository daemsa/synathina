...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "teamstat.cancel" || document.formvalidator.isValid(document.getElementById("teamstat-form")))
		{
			' . $this->form->getField('misc')->save() . '

			if (window.opener && (task == "teamstat.save" || task == "teamstat.cancel"))
			{
				window.opener.document.closeEditWindow = self;
				window.opener.setTimeout("window.document.closeEditWindow.close()", 1000);
			}

			Joomla.submitform(task, document.getElementById("teamstat-form"));
		}
	};
');
...
<div class="pull-right">
	<button class="btn btn-primary" type="button" onclick="Joomla.submitbutton('teamstat.apply');"><?php echo JText::_('JTOOLBAR_APPLY') ?></button>
	<button class="btn btn-primary" type="button" onclick="Joomla.submitbutton('teamstat.save');"><?php echo JText::_('JTOOLBAR_SAVE') ?></button>
	<button class="btn" type="button" onclick="Joomla.submitbutton('teamstat.cancel');"><?php echo JText::_('JCANCEL') ?></button>
</div>

<div class="clearfix"> </div>
<hr class="hr-condensed" />

<form action="<?php echo JRoute::_('index.php?option=com_teamstats&layout=modal&tmpl=component&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="teamstat-form" class="form-validate">
	...
	<div class="form-horizontal">
		...
		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'details', empty($this->item->id) ? 'New teamstat' : 'Edit teamstat'); ?>
		<div class="row-fluid">
			<div class="span9">
