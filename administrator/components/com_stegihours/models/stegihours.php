...
class StegihoursModelStegihours extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('a.date_end', 'desc');
	}
		...
		$query->select(
			$db->quoteName(
				explode(', ', $this->getState(
					'list.select',
					'a.id, a.date_end, a.date_start, a.name, a.team_id, a.description, a.language, a.ordering, a.alias, a.published, a.title, a.description, a.catid, a.checked_out, a.created_by, a.publish_up, a.publish_down'
					)
				)
			)
		);
		$query->from($db->quoteName('#__stegihours', 'a'));
		...
		if ($assoc)
		{
			$query->select('COUNT(' . $db->quoteName('asso2.id') . ') > 1 as ' . $db->quoteName('association'))
				->join(
					...
					. ' AND ' . $db->quoteName('asso.context') . ' = ' . $db->quote('com_stegihours.stegihour')
				)
				->join(
