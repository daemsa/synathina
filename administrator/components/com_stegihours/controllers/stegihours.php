...
class StegihoursControllerStegihours extends JControllerAdmin
{
	...
	public function unpublish()
	{
		...
		foreach ($_REQUEST["cid"] as $cid){
			$db->setQuery("UPDATE #__stegihours SET published='0' WHERE id='".intval($cid)."'");
			if(!$db->query()){
	...
	public function publish($pks, $value = 1)
	{
		...
		elseif($_REQUEST['task']=='delete'){
			...
			foreach ($_REQUEST["cid"] as $cid){
				$db->setQuery("UPDATE #__stegihours SET published='".$publishvalue."' WHERE id='".intval($cid)."'");
				if(!$db->query()){
					JError::raiseWarning(100, $db->getError());
					$mainframe->redirect("index.php?option=com_stegihours&view=stegihours");
				}
	...
	public function trash($pks, $value = 1)
	{
		...
		foreach ($_REQUEST["cid"] as $cid){
			$db->setQuery("UPDATE #__stegihours SET published='-2' WHERE id='".intval($cid)."'");
			if(!$db->query()){
				JError::raiseWarning(100, $db->getError());
				$mainframe->redirect("index.php?option=com_stegihours&view=stegihours");
			}
	...
	public function delete()
	{
		...
		foreach ($_REQUEST["cid"] as $cid){
			$db->setQuery("DELETE FROM #__stegihours WHERE id='".intval($cid)."'");
			if(!$db->query()){
				JError::raiseWarning(100, $db->getError());
				$mainframe->redirect("index.php?option=com_stegihours&view=stegihours");
			}
