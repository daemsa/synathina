...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "teamsexport.cancel" || document.formvalidator.isValid(document.getElementById("teamsexport-form")))
		{
			' . $this->form->getField('misc')->save() . '

			if (window.opener && (task == "teamsexport.save" || task == "teamsexport.cancel"))
			{
				window.opener.document.closeEditWindow = self;
				window.opener.setTimeout("window.document.closeEditWindow.close()", 1000);
			}

			Joomla.submitform(task, document.getElementById("teamsexport-form"));
		}
	};
');
...
<div class="pull-right">
	<button class="btn btn-primary" type="button" onclick="Joomla.submitbutton('teamsexport.apply');"><?php echo JText::_('JTOOLBAR_APPLY') ?></button>
	<button class="btn btn-primary" type="button" onclick="Joomla.submitbutton('teamsexport.save');"><?php echo JText::_('JTOOLBAR_SAVE') ?></button>
	<button class="btn" type="button" onclick="Joomla.submitbutton('teamsexport.cancel');"><?php echo JText::_('JCANCEL') ?></button>
</div>

<div class="clearfix"> </div>
<hr class="hr-condensed" />

<form action="<?php echo JRoute::_('index.php?option=com_teamsexports&layout=modal&tmpl=component&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="teamsexport-form" class="form-validate">
	...
	<div class="form-horizontal">
		...
		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'details', empty($this->item->id) ? 'New teamsexport' : 'Edit teamsexport'); ?>
		<div class="row-fluid">
			<div class="span9">
