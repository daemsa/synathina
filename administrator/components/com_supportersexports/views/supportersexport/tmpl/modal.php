...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "supportersexport.cancel" || document.formvalidator.isValid(document.getElementById("supportersexport-form")))
		{
			' . $this->form->getField('misc')->save() . '

			if (window.opener && (task == "supportersexport.save" || task == "supportersexport.cancel"))
			{
				window.opener.document.closeEditWindow = self;
				window.opener.setTimeout("window.document.closeEditWindow.close()", 1000);
			}

			Joomla.submitform(task, document.getElementById("supportersexport-form"));
		}
	};
');
...
<div class="pull-right">
	<button class="btn btn-primary" type="button" onclick="Joomla.submitbutton('supportersexport.apply');"><?php echo JText::_('JTOOLBAR_APPLY') ?></button>
	<button class="btn btn-primary" type="button" onclick="Joomla.submitbutton('supportersexport.save');"><?php echo JText::_('JTOOLBAR_SAVE') ?></button>
	<button class="btn" type="button" onclick="Joomla.submitbutton('supportersexport.cancel');"><?php echo JText::_('JCANCEL') ?></button>
</div>

<div class="clearfix"> </div>
<hr class="hr-condensed" />

<form action="<?php echo JRoute::_('index.php?option=com_supportersexports&layout=modal&tmpl=component&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="supportersexport-form" class="form-validate">
	...
	<div class="form-horizontal">
		...
		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'details', empty($this->item->id) ? 'New supportersexport' : 'Edit supportersexport'); ?>
		<div class="row-fluid">
			<div class="span9">
