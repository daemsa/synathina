...
/**
 * Core Update Notification plugin
 *
 * Sends out an email to all Super Users or a predefined email address when a new Core version is available.
 *
 ...
 * GPLv3 or later) to allow its inclusion in the Core CMS.
 *
 * @since  3.5
 */
class PlgSystemUpdatenotification extends JPlugin
{
	...
	{
		// Get the timeout for Core updates, as configured in com_installer's component parameters
		...
		// This is the extension ID for Core itself
		$eid = 700;
		...
		// Unfortunately Core MVC doesn't allow us to autoload classes, hence the need for an ugly require_once
		require_once JPATH_ADMINISTRATOR . '/components/com_installer/models/update.php';

		// Get the update model and retrieve the Core core updates
		...
		// If we're here, we have updates. First, get a link to the Core Update component.
		$baseURL  = JUri::base();
		 ...
		 * site, confusing them (they can't understand the third party security solution is not part of Core proper).
		 ...
		 * update email which would make Core seem bad. So, please, if you don't fully understand what the
